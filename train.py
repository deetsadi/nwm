...
def main(args):
    ...
    train_dataset = RawVideoDataset(
                data_dir=f"{data_config["data_folder"]}/{data_config["train"]}",
                image_size=config["image_size"],
                context_frames=config["context_size"],
                skip_frame=config["skip_frame"],
                n_goals=data_config["goals_per_obs"]
    )
    print (f"Number of examples in train ds: {len(train_dataset)}")
    test_dataset = RawVideoDataset(
                data_dir=f"{data_config["data_folder"]}/{data_config["test"]}",
                image_size=config["image_size"],
                context_frames=config["context_size"],
                skip_frame=config["skip_frame"],
                n_goals=1 # always eval at the 77th frame
    )
    ...
    logger.info(f"Dataset contains {len(train_dataset):,} examples")
    ...
    # Start a run
    wandb.init(
        project="nwm",  # change this to your project name
        name=config["run_name"],            # optional: name your run
        config={
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t in loader:
            ...
            with torch.amp.autocast('cuda', enabled=bfloat_enable, dtype=torch.bfloat16):
                ...
                if data_config["goals_per_obs"] == 1:
                    x = rearrange(x, 'B C T H W -> B T C H W')
                else:
                    x = rearrange(x.flatten(0, 1), 'B C T H W -> B T C H W')
                    y = y.flatten(0, 1)
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    ...
                    if train_steps % (args.ckpt_every * 2) == 0 and train_steps > 0:
            ...
            if train_steps % args.eval_every == 0 and train_steps > 0:
                ...
                sim_score, psnr = evaluate(ema, tokenizer, diffusion, test_dataset, rank, config["batch_size"], config["num_workers"], latent_size, device, save_dir, args.global_seed, bfloat_enable, num_cond)
                dist.barrier()
                eval_end_time = time()
                eval_time = eval_end_time - eval_start_time
                logger.info(f"(step={train_steps:07d}) Perceptual Loss: {sim_score:.4f}, PSNR: {psnr}, Eval Time: {eval_time:.2f}")
...
def calculate_batch_psnr(img1, img2, max_val=1.0):
    # img1, img2: [B, C, H, W]
    mse = F.mse_loss(img1, img2, reduction='none')
    mse = mse.mean(dim=[1, 2, 3])  # Mean over C, H, W per image in batch

    # Avoid log of zero
    psnr = 20 * torch.log10(max_val / torch.sqrt(mse + 1e-8))
    return psnr  # Returns [B] tensor of PSNR values per image
...
def evaluate(model, vae, diffusion, test_dataloaders, rank, batch_size, num_workers, latent_size, device, save_dir, seed, bfloat_enable, num_cond):
    ...
    for x, y, rel_t in loader:
        ...
        with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
            ...
            # PSNR calculation
            mse = F.mse_loss(samples, x_start_pixels, reduction='none').mean(dim=[1, 2, 3])  # Per-image MSE
            batch_psnr = 20 * torch.log10(1.0 / torch.sqrt(mse + 1e-8))
            psnr += batch_psnr.sum()
